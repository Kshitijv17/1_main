    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p class="mb-1"><strong>Name:</strong> {{ $order->customer_name ?? $order->user_name ?? 'N/A' }}</p>
                            <p class="mb-1"><strong>Email:</strong> {{ $order->customer_email ?? $order->user_email ?? 'N/A' }}</p>
                            <p class="mb-1"><strong>Phone:</strong> {{ $order->customer_phone ?? $order->user_phone ?? 'N/A' }}</p>
                        </div>
                        <div class="col-md-6">
                            ...
                            @if($shippingAddress && is_array($shippingAddress))
                                <p class="mb-1">{{ $shippingAddress['street'] ?? 'N/A' }}</p>
                                <p class="mb-1">{{ ($shippingAddress['city'] ?? '') . ', ' . ($shippingAddress['state'] ?? '') }}</p>
                                <p class="mb-1">{{ ($shippingAddress['zip'] ?? '') . ' ' . ($shippingAddress['country'] ?? '') }}</p>
                            @else
                                <p class="mb-1 text-muted">No shipping address provided</p>
                            @endif
                        </div>
                    </div>
                </div>
                    ...
                    @foreach($order->items as $item)
                        <div class="d-flex align-items-center py-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                            ...
                            <div class="flex-grow-1">
                                <h6 class="mb-1">{{ $item->product->title ?? $item->product_name ?? 'Product' }}</h6>
                                <p class="text-muted mb-1">Quantity: {{ $item->quantity }}</p>
                                <p class="text-muted mb-0">Price: ${{ number_format($item->unit_price, 2) }} each</p>
                            </div>
                            <div class="text-end">
                                <h6 class="mb-0">${{ number_format($item->total_price, 2) }}</h6>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="text-center">
                <a href="{{ route('user.home') }}" class="btn btn-primary btn-lg me-3">
                    <i class="fas fa-home me-2"></i>Continue Shopping
                </a>
                <a href="{{ route('user.order.details', $order) }}" class="btn btn-outline-primary btn-lg">
