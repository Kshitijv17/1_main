...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $stats = [
            ...
            'completed_orders' => Order::delivered()->count(), // Completed = Delivered
            'cancelled_orders' => Order::cancelled()->count(),
            'total_revenue' => Order::where('payment_status', 'paid')->sum('total_amount'),
            'pending_payments' => Order::where('payment_status', 'pending')->sum('total_amount'), // Renamed from pending_revenue
        ];

        // Get status options for filters
        $statuses = array_keys(Order::getStatuses());
        $paymentStatuses = array_keys(Order::getPaymentStatuses());
        $shops = Shop::select('id', 'name')->get();

        return view('admin.orders.index', compact('orders', 'stats', 'statuses', 'paymentStatuses', 'shops'));
    }
    ...
    {
        $order->load(['user', 'items.product']);
        $users = User::whereIn('role', ['user', 'guest'])->orderBy('name')->get();
